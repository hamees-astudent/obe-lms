...
The backend includes a `seed` command that fills the database with realistic demo data for presentations:

- 4 programs: BS CS, BS SE, BS AI and MS CS, each with PLOs and an HEC-style curriculum of real courses (codes such as `CS-201`) with CLOs and CLO→PLO mappings.
- Semesters from **Spring 2022 to Fall 2026**. Spring runs 8 Jan – 23 Jun and Fall 8 Jul – 31 Dec. Fall 2026 is the current open semester; all earlier ones are closed.
- One intake per year in Spring, with 30 students per program. That gives 600 students, each with a roll number of the form `YY110000CNNN`, e.g. `241100001001`.
- Each intake is a **cohort**, and cohorts are enrolled into their semester's course offerings.
- About 28 teachers and 12 teaching assistants, all with Pakistani names.
- For every offering:
  - course materials and announcements (mid-terms, presentations, holidays, university events)
  - quizzes with auto-graded submissions
  - graded assignments
  - weekly attendance
  - mid-term and final exams with per-question marks mapped to CLOs
- Transcripts for every closed semester, generated by the app's own transcript service, plus grading scales and event publication records.
- 12 lecture rooms and 8 computing labs, and a clash-free weekly **timetable** for the current term, generated by the app's own timetable generator. Current-term attendance sessions follow the timetable: they fall on the timetabled days, at the timetabled times, and one is taken for every lecture and lab.

In the open semester, activity is generated only up to today's date: past quizzes are submitted, the mid-term is marked, and the final exam is still a draft.
...
# Refuses to run if demo data already exists
./mvnw spring-boot:run -Dspring-boot.run.arguments="seed"

# Deletes ALL data in every application table, then seeds
./mvnw spring-boot:run -Dspring-boot.run.arguments="seed --clean"

# Or from the packaged jar
java -jar target/lms-backend-0.0.1-SNAPSHOT.jar seed --clean
```

> **Warning:** `--clean` truncates every table except `flyway_schema_history`, including users you created yourself.

The command needs the same PostgreSQL, Redis, Kafka and MinIO as the app. It does not start the web server or the Kafka consumers, and it publishes no events. The whole run is one transaction, so a failure leaves the database unchanged. It takes about 1–2 minutes and then prints row counts and sample logins:

| Role | Login |
|---|---|
| Admin | `admin@lms.local` |
| Teacher | `firstname.lastname@lms.edu.pk` (the command prints the busiest teacher this term) |
| Student | `<roll number>@student.lms.edu.pk`, e.g. `241100001001@student.lms.edu.pk` |

Any extra `--property=value` argument is passed to Spring. For example, `--spring.datasource.url=jdbc:postgresql://localhost:5432/other_db` seeds a different database.
